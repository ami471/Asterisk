...
sub __welcome
{
	...
	if ($time_display >= '000000' && $time_display < '090000')
        {
         ...
         if($mem_obj->{'did'} == 8610)
         {
                ...
                $mem_obj->{'did_mobile'} = '8976922980';
                $mem_obj->{'contact_1'}  = '8097552730';
                $mem_obj->{'campaign'}   =  'Moho Mumbai';

         }
          if($mem_obj->{'did'} == 8620)
         {
                
                $mem_obj->{'did_mobile'} = '8976922975';
                $mem_obj->{'contact_1'}  = '8097584642';
                $mem_obj->{'campaign'}   =  'Moho Pune';

         }
          if($mem_obj->{'did'} == 8623)
         {
                
                $mem_obj->{'did_mobile'} = '8976922985';
                $mem_obj->{'contact_1'}  = '8287386977';
                $mem_obj->{'campaign'}   =  'Moho Delhi';

         }
          if($mem_obj->{'did'} == 8628)
         {
                
                $mem_obj->{'did_mobile'} = '8976922990';
                $mem_obj->{'contact_1'}  = '7208826857';
                $mem_obj->{'campaign'}   = 'Moho Bengaluru';

         }
        if($mem_obj->{'did'} == 8629)
         {
                
                $mem_obj->{'did_mobile'} = '8976922995';
                $mem_obj->{'contact_1'}  = '7569202073';
                $mem_obj->{'campaign'}   =  'Moho Hydrabad';
         ...
         if($mem_obj->{'did'} == 8664)
         {
                ...
                $mem_obj->{'did_mobile'} = '8976923001';
                $mem_obj->{'contact_1'}  =  '8939914848';
                $mem_obj->{'campaign'}   =  'Moho Chennai';

         }
        if($mem_obj->{'did'} == 8680)
         {
                
                $mem_obj->{'did_mobile'} = '8976923005';
                $mem_obj->{'contact_1'}  =  '9426769226';
                $mem_obj->{'campaign'}   =  'Moho Ahmedabad';
	...
	$mem_obj->{'time_now'} =  $time_now;
	 my $mobileno_length = length($mem_obj->{'callerid'});
        ...
        my $tablename = 'digitalmedia_moho';
        my $dbh = DBI->connect("dbi:mysql:database=asterisk;host=localhost","root","mys\@r0ja");
        my $count =0;
        my $header = 'fsd';
        my $query = "INSERT INTO $tablename (mobile_no,date,time,day,Time_Session,campaign,weekday,media_vehicle,media_channel,agent_number,did) VALUES('$callerid','$current_date','$current_time','$weekday','$time_now','$mem_obj->{'campaign'}','$day_is','$mem_obj->{'campaign'}','$mem_obj->{'campaign'}','$mem_obj->{'contact_1'}','$mem_obj->{'did_mobile'}')";
